@php
    ...
    $attendedAt = optional($record->attended_at)->format('F j, Y \a\t h:i A') ?? '-';
@endphp
    ...
    <div class="flex-1 w-full">
        <table class="w-full text-left table-auto">
            <tbody>
                ...
                <tr>
                    <td colspan="2" class="py-4 font-semibold">Greeting</td>
                </tr>
                <tr>
                    <td colspan="2" class="py-4">
                        @if ($record->greeting_wall_image_url)
                            <img
                                src="{{ Storage::disk('minio')->temporaryUrl($record->greeting_wall_image_url, now()->addMinutes(5)) }}"
                                alt="Greeting Wall Photo"
